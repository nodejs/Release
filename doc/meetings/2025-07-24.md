...
* Proposal for new release schedule / users are not interested in releases that will not become LTS  [#953](https://github.com/nodejs/Release/issues/953)
  ...
  * Michael: we would have to lock the main branch in order to make that work, so taht we can cherry-pick specific commits from the private repo without conflicts
